...
public:
	...
	int connect()
	{
		...
		if (fDebugMode)
		{
			std::cerr << "RemoteTestRunner: trying to connect to " << fHost << ":" << fPort
					<< std::endl;
		}
	...
	void runTests()
	{
		...
		sendTree(suite);
		if (fDebugMode)
			std::cerr << "done send tree - time(ms): " << currentTimeMillis() - startTime
					<< std::endl; //$NON-NLS-1$
		...
		if (typeid(*test) == typeid(CppUnit::TestDecorator))
				{
					...
					void addFailure(const CppUnit::TestFailure &failure)
					{
						...
						{
							notifyTestFailed(TEST_FAILED, failure.failedTestName(), getTrace(
									failure));
						}
					}
					// From TestListener
